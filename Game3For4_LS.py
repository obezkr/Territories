...
a=Tk()
a.geometry('880x600')
a.title('Territories - Last Stand')
WIDTH=600
HEIGHT=600
COLOR1='blue'
COLOR2='red'
...
##########
            ...
            while y2<yPutRect+step:
                while x2<xPutRect+step:
                    l2=[x2, y2]                     ### ошибка в удалении ненужных елементов массива, и, возможно, проверке (исправлено) ###
                    if COLOR1=='blue':
                        if l2 not in LLB:
                            LLB.append(l2)
...
#################
    ...
    if InGameB==1:
        chch=1
        while (chch<len(LLLB)):
            c.create_rectangle(LLLB[chch][0], LLLB[chch][1], LLLB[chch][0]+step, LLLB[chch][1]+step, outline='black', fill='#c4c4c4')
            qq=LL1.index(LLLB[chch])
            del LL1[qq]
            k=k-1
            del LLLB[chch]
        c.delete(Rect)
    ...
    if InGameR==1:
        chch=1
        while (chch<len(LLLR)):
            c.create_rectangle(LLLR[chch][0], LLLR[chch][1], LLLR[chch][0]+step, LLLR[chch][1]+step, outline='black', fill='#c4c4c4')
            qq=LL1.index(LLLR[chch])
            del LL1[qq]
            k=k-1
            del LLLR[chch]
        c.delete(Rect)
    ...
    if InGameY==1:
        chch=1
        while (chch<len(LLLY)):
            c.create_rectangle(LLLY[chch][0], LLLY[chch][1], LLLY[chch][0]+step, LLLY[chch][1]+step, outline='black', fill='#c4c4c4')
            qq=LL1.index(LLLY[chch])
            del LL1[qq]
            k=k-1
            del LLLY[chch]
        c.delete(Rect)
    ...
    if InGameG==1:
        chch=1
        while (chch<len(LLLG)):
            c.create_rectangle(LLLG[chch][0], LLLG[chch][1], LLLG[chch][0]+step, LLLG[chch][1]+step, outline='black', fill='#c4c4c4')
            qq=LL1.index(LLLG[chch])
            del LL1[qq]
            k=k-1
            del LLLG[chch]
        c.delete(Rect)
